...
inline void PrintValidatorPointers(
    unsigned depth, const wchar_t* s, const wchar_t* d)
{
    wprintf(
...
private:
    ...
    static uint64_t Hash(uint64_t h, uint64_t d)
    {
        static const uint64_t kPrime
            = RAPIDJSON_UINT64_C2(0x00000100, 0x000001b3);
        h ^= d;
        h *= kPrime;
        return h;
    }
...
public:
        ...
        , exclusiveMaximum_(false)
    {
        ...
        if (const ValueType* v = GetMember(value, GetEnumString()))
            if (v->IsArray() && v->Size() > 0) {
                ...
                for (ConstValueIterator itr = v->Begin(); itr != v->End();
                    ++itr) {
                    typedef Hasher<EncodingType, MemoryPoolAllocator<>>
                        EnumHasherType;
                    char buffer[256 + 24];
                    MemoryPoolAllocator<> hasherAllocator(
        ...
        if (schemaDocument) {
            AssignIfExist(
                allOf_, *schemaDocument, p, value, GetAllOfString(), document);
            AssignIfExist(
                anyOf_, *schemaDocument, p, value, GetAnyOfString(), document);
            AssignIfExist(
                oneOf_, *schemaDocument, p, value, GetOneOfString(), document);
        }

        if (const ValueType* v = GetMember(value, GetNotString())) {
            schemaDocument->CreateSchema(
                &not_, p.Append(GetNotString(), allocator_), *v, document);
            ...
            = GetMember(value, GetDependenciesString());
        {
            ...
            if (required && required->IsArray())
                for (ConstValueIterator itr = required->Begin();
        ...
        if (const ValueType* v
            ...
            for (ConstMemberIterator itr = v->MemberBegin();
                itr != v->MemberEnd(); ++itr) {
                new (&patternProperties_[patternPropertyCount_])
                    PatternProperty();
                ...
                if (FindPropertyIndex(itr->name, &sourceIndex)) {
                    if (itr->value.IsArray()) {
                        properties_[sourceIndex].dependencies
                            = static_cast<bool*>(allocator_->Malloc(
                        ...
                        schemaDocument->CreateSchema(
                            ...
                            q.Append(itr->name, allocator_), itr->value,
                            document);
        ...
        if (const ValueType* v = GetMember(value, GetAdditionalItemsString())) {
            if (v->IsBool())
                ...
                schemaDocument->CreateSchema(&additionalItemsSchema_,
                    p.Append(GetAdditionalItemsString(), allocator_), *v,
                    document);
        }
    ...
    RAPIDJSON_FORCEINLINE bool EndValue(Context& context) const
    {
        if (context.patternPropertiesValidatorCount > 0) {
            ...
            if (context.objectPatternValidatorType
                ...
                otherValid
                    = context.patternPropertiesValidators[--count]->IsValid();
        ...
        if (allOf_.schemas)
            for (SizeType i = allOf_.begin; i < allOf_.begin + allOf_.count;
                i++)
                if (!context.validators[i]->IsValid())
                    RAPIDJSON_INVALID_KEYWORD_RETURN(GetAllOfString());

        if (anyOf_.schemas) {
            for (SizeType i = anyOf_.begin; i < anyOf_.begin + anyOf_.count;
                i++)
                if (context.validators[i]->IsValid())
                    goto foundAny;
        ...
        if (oneOf_.schemas) {
            bool oneValid = false;
            for (SizeType i = oneOf_.begin; i < oneOf_.begin + oneOf_.count;
                i++)
                if (context.validators[i]->IsValid()) {
                    if (oneValid)
    ...
    bool String(Context& context, const Ch* str, SizeType length, bool) const
    {
        ...
        if (minLength_ != 0 || maxLength_ != SizeType(~0)) {
            SizeType count;
            if (internal::CountStringCodePoint<EncodingType>(
                    str, length, &count)) {
                if (count < minLength_)
    ...
    bool StartObject(Context& context) const
    {
        ...
        if (hasDependencies_ || hasRequired_) {
            ...
            std::memset(
                context.propertyExist, 0, sizeof(bool) * propertyCount_);
        }

        if (patternProperties_) { // pre-allocate schema array
            SizeType count = patternPropertyCount_
                + 1; // extra for valuePatternValidatorType
        ...
        if (patternProperties_) {
            ...
            for (SizeType i = 0; i < patternPropertyCount_; i++)
                if (patternProperties_[i].pattern
                    ...
                    context.patternPropertiesSchemas[context
                            .patternPropertiesSchemaCount++]
            ...
            for (SizeType sourceIndex = 0; sourceIndex < propertyCount_;
                ...
                if (context.propertyExist[sourceIndex]) {
                    if (properties_[sourceIndex].dependencies) {
                        for (SizeType targetIndex = 0;
                            targetIndex < propertyCount_; targetIndex++)
                            if (properties_[sourceIndex]
                                    .dependencies[targetIndex]
                        ...
                        if (!context
                                .validators[properties_[sourceIndex]
...
#define RAPIDJSON_STRING_(name, ...)                                           \
    ...
    RAPIDJSON_STRING_(AdditionalProperties, 'a', 'd', 'd', 'i', 't', 'i', 'o',
        'n', 'a', 'l', 'P', 'r', 'o', 'p', 'e', 'r', 't', 'i', 'e', 's')
    RAPIDJSON_STRING_(MinProperties, 'm', 'i', 'n', 'P', 'r', 'o', 'p', 'e',
        'r', 't', 'i', 'e', 's')
    RAPIDJSON_STRING_(MaxProperties, 'm', 'a', 'x', 'P', 'r', 'o', 'p', 'e',
        'r', 't', 'i', 'e', 's')
    ...
    RAPIDJSON_STRING_(AdditionalItems, 'a', 'd', 'd', 'i', 't', 'i', 'o', 'n',
        'a', 'l', 'I', 't', 'e', 'm', 's')
    RAPIDJSON_STRING_(
        UniqueItems, 'u', 'n', 'i', 'q', 'u', 'e', 'I', 't', 'e', 'm', 's')
    ...
    RAPIDJSON_STRING_(ExclusiveMaximum, 'e', 'x', 'c', 'l', 'u', 's', 'i', 'v',
        'e', 'M', 'a', 'x', 'i', 'm', 'u', 'm')
    RAPIDJSON_STRING_(
        MultipleOf, 'm', 'u', 'l', 't', 'i', 'p', 'l', 'e', 'O', 'f')
...
private:
    ...
    {
        static SchemaType typeless(0, PointerType(),
            ValueType(kObjectType).Move(), ValueType(kObjectType).Move(), 0);
        return &typeless;
    }
    ...
    {
        for (typename V1::ConstValueIterator itr = a.Begin(); itr != a.End();
            ++itr)
            if (*itr == v)
                return;
        V1 c(v, *allocator_);
    ...
    static const ValueType* GetMember(
        const ValueType& value, const ValueType& name)
    {
        typename ValueType::ConstMemberIterator itr = value.FindMember(name);
        return itr != value.MemberEnd() ? &(itr->value) : 0;
    }

    static void AssignIfExist(
        bool& out, const ValueType& value, const ValueType& name)
    {
    ...
    static void AssignIfExist(
        SizeType& out, const ValueType& value, const ValueType& name)
    {
        if (const ValueType* v = GetMember(value, name))
    ...
    void AssignIfExist(SchemaArray& out, SchemaDocumentType& schemaDocument,
        const PointerType& p, const ValueType& value, const ValueType& name,
        ...
        if (const ValueType* v = GetMember(value, name)) {
            if (v->IsArray() && v->Size() > 0) {
                ...
                for (SizeType i = 0; i < out.count; i++)
                    schemaDocument.CreateSchema(&out.schemas[i],
                        q.Append(i, allocator_), (*v)[i], document);
    ...
    static bool IsPatternMatch(
        const RegexType* pattern, const Ch* str, SizeType)
    {
        ...
        if (value.IsString())
            try {
                return new (allocator_->Malloc(sizeof(RegexType))) RegexType(
                    value.GetString(), std::size_t(value.GetStringLength()),
    ...
    static bool IsPatternMatch(
        const RegexType* pattern, const Ch* str, SizeType length)
    {
            ...
            if (hasSchemaDependencies_) {
                for (SizeType i = 0; i < propertyCount_; i++)
                    if (properties_[i].dependenciesSchema)
                        context.validators[properties_[i]
                                .dependenciesValidatorIndex]
    ...
    void CreateSchemaValidators(
        Context& context, const SchemaArray& schemas) const
    {
    ...
    bool CheckInt(Context& context, int64_t i) const
    {
        ...
        if (!minimum_.IsNull()) {
            if (minimum_.IsInt64()) {
                ...
                RAPIDJSON_INVALID_KEYWORD_RETURN(
                    GetMinimumString()); // i <= max(int64_t) <
                                         // minimum.GetUint64()
        ...
        if (!multipleOf_.IsNull()) {
            if (multipleOf_.IsUint64()) {
                if (static_cast<uint64_t>(i >= 0 ? i : -i)
                        % multipleOf_.GetUint64()
                    != 0)
...
template <typename Stack, typename Ch> struct TokenHelper {
    RAPIDJSON_FORCEINLINE static void AppendIndexToken(
        Stack& documentStack, SizeType index)
    {
        *documentStack.template Push<Ch>() = '/';
        char buffer[21];
        size_t length = static_cast<size_t>(
            (sizeof(SizeType) == 4 ? u32toa(index, buffer)
                                   : u64toa(index, buffer))
            - buffer);
...
template <typename Stack> struct TokenHelper<Stack, char> {
        ...
        Stack& documentStack, SizeType index)
    {
        if (sizeof(SizeType) == 4) {
            char* buffer
                = documentStack.template Push<char>(1 + 10); // '/' + uint
            *buffer++ = '/';
            ...
            char* buffer
                = documentStack.template Push<char>(1 + 20); // '/' + uint64
            *buffer++ = '/';
...
public:
    ...
    /*!
        ...
        \param remoteProvider An optional remote schema document provider for
       resolving remote reference. Can be null. \param allocator An optional
       allocator instance for allocating memory. Can be null.
    */
        ...
        , schemaRef_(allocator, kInitialSchemaRefSize)
    {
        ...
        // Generate root schema, it will call CreateSchema() to create
        // sub-schemas, And call AddRefSchema() if there are $ref.
        CreateSchemaRecursive(&root_, PointerType(), document, document);

        // Resolve $ref
        while (!schemaRef_.Empty()) {
            SchemaRefEntry* refEntry
...
private:
    ...
    struct SchemaEntry {
        SchemaEntry(
            const PointerType& p, SchemaType* s, bool o, Allocator* allocator)
            : pointer(p, allocator)
            , schema(s)
            , owned(o)
        {
        ...
        if (v.GetType() == kObjectType) {
            ...
            for (SizeType i = 0; i < v.Size(); i++)
                CreateSchemaRecursive(
                    0, pointer.Append(i, allocator_), v[i], document);
    }
                ...
                if (i > 0) { // Remote reference, resolve immediately
                    if (remoteProvider_) {
                        if (const GenericSchemaDocument* remoteDocument
                            = remoteProvider_->GetRemoteDocument(s, i - 1)) {
                            PointerType pointer(&s[i], len - i, allocator_);
                            if (pointer.IsValid()) {
                                if (const SchemaType* sc
                                    = remoteDocument->GetSchema(pointer)) {
                                    if (schema)
                                        *schema = sc;
                                    return true;
                                }
    ...
    {
        for (const SchemaEntry* target
            = schemaMap_.template Bottom<SchemaEntry>();
...
public:
    ...
    /*!
       ...
       states. \param schemaStackCapacity Optional initial capacity of schema
       path stack. \param documentStackCapacity Optional initial capacity of
       document path stack.
    */
    ...
    */
    GenericSchemaValidator(const SchemaDocumentType& schemaDocument,
        OutputHandler& outputHandler, StateAllocator* allocator = 0,
        size_t schemaStackCapacity = kDefaultSchemaStackCapacity,
        size_t documentStackCapacity = kDefaultDocumentStackCapacity)
        : schemaDocument_(&schemaDocument)
        , root_(schemaDocument.GetRoot())
    ...
    {
        return schemaStack_.Empty()
            ? PointerType()
            : schemaDocument_->GetPointer(&CurrentSchema());
    }
    ...
    PointerType GetInvalidDocumentPointer() const
    {
        return documentStack_.Empty()
            ? PointerType()
            : PointerType(documentStack_.template Bottom<Ch>(),
...
#define RAPIDJSON_SCHEMA_HANDLE_PARALLEL_(method, arg2)                        \
    for (Context* context = schemaStack_.template Bottom<Context>();           \
        ...
        if (context->patternPropertiesValidators)                              \
            for (SizeType i_ = 0;                                              \
                i_ < context->patternPropertiesValidatorCount; i_++)           \
...
private:
    ...
    {
        if (!stateAllocator_)
            stateAllocator_ = ownStateAllocator_
    ...
    bool BeginValue()
    {
        if (schemaStack_.Empty())
            ...
            if (count > 0) {
                CurrentContext().objectPatternValidatorType
    ...
    bool EndValue()
    {
        ...
        *documentStack_.template Push<Ch>() = '\0';
        ...
        internal::PrintValidatorPointers(
            depth_, sb.GetString(), documentStack_.template Bottom<Ch>());
#endif
        ...
        if (!schemaStack_.Empty()) {
            ...
            if (context.valueUniqueness) {
                HashCodeArray* a = static_cast<HashCodeArray*>(
                ...
                for (typename HashCodeArray::ConstValueIterator itr
                    = a->Begin();
                    itr != a->End(); ++itr)
                    if (itr->GetUint64() == h)
                        RAPIDJSON_INVALID_KEYWORD_RETURN(
    ...
    {
        documentStack_.template Reserve<Ch>(1
            + len
                * 2); // worst case all characters are escaped as two characters
    ...
    */
    ...
    template <typename Handler> bool operator()(Handler& handler)
    {
        GenericReader<SourceEncoding, typename SchemaDocumentType::EncodingType,
            StackAllocator>
            reader;
        GenericSchemaValidator<SchemaDocumentType, Handler> validator(
            sd_, handler);
