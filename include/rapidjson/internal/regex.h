...
private:
    ...
    template <typename InputStream> void Parse(DecodedStream<InputStream>& ds)
    {
        ...
        Stack<Allocator> atomCountStack(
            &allocator, 256); // unsigned (Atom per parenthesis)
    ...
    {
        SizeType s
            = NewState(kRegexInvalidState, kRegexInvalidState, codepoint);
        *operandStack.template Push<Frag>() = Frag(s, s, s);
    }

    void ImplicitConcatenation(
        Stack<Allocator>& atomCountStack, Stack<Allocator>& operatorStack)
    {
    ...
    bool Eval(Stack<Allocator>& operandStack, Operator op)
    {
        ...
        case kAlternation:
            if (operandStack.GetSize() >= sizeof(Frag) * 2) {
                ...
                *operandStack.template Push<Frag>() = Frag(
    ...
    bool EvalQuantifier(Stack<Allocator>& operandStack, unsigned n, unsigned m)
    {
        ...
        for (unsigned i = 0; i < n - 1; i++)
            Eval(operandStack, kConcatenation); // a{3} -> aaa, a{3,} -> aaa+,
                                                // a{3.5} -> aaaa?a?
    ...
    {
        const Frag src
            = *operandStack.template Top<Frag>(); // Copy constructor to prevent
                                                  // invalidation
        SizeType count
            = stateCount_ - src.minIndex; // Assumes top operand contains states
                                          // in [src->minIndex, stateCount_)
    ...
    template <typename InputStream>
    bool CharacterEscape(
        DecodedStream<InputStream>& ds, unsigned* escapedCodepoint)
    {
        unsigned codepoint;
        switch (codepoint = ds.Take()) {
        case '^':
    ...
    bool SearchWithAnchoring(
        InputStream& is, bool anchorBegin, bool anchorEnd) const
    {
    ...
    bool AddState(Stack<Allocator>& l, SizeType index) const
    {
        ...
        return s.out
            == kRegexInvalidState; // by using PushUnsafe() above, we can ensure
                                   // s is not validated due to reallocation.
    }
    ...
    bool MatchRange(SizeType rangeIndex, unsigned codepoint) const
    {
        ...
        while (rangeIndex != kRegexInvalidRange) {
            ...
            if (codepoint >= (r.start & ~kRangeNegationFlag)
                && codepoint <= r.end)
                return yes;
            rangeIndex = r.next;
        }
