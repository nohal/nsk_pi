...
class GenericMemberIterator
    : public std::iterator<std::random_access_iterator_tag,
          typename internal::MaybeAddConst<Const,
              GenericMember<Encoding, Allocator>>::Type> {
...
#ifndef __clang__ // -Wdocumentation
    /*! \param str constant string, lifetime assumed to be longer than the use
       of the string in e.g. a GenericValue \param len length of the string,
...
template <typename ValueType> struct TypeHelper<ValueType, bool> {
    ...
    static ValueType& Set(
        ValueType& v, bool data, typename ValueType::AllocatorType&)
    {
...
template <typename ValueType> struct TypeHelper<ValueType, int> {
    ...
    static ValueType& Set(
        ValueType& v, int data, typename ValueType::AllocatorType&)
    {
...
template <typename ValueType> struct TypeHelper<ValueType, unsigned> {
    ...
    static ValueType& Set(
        ValueType& v, unsigned data, typename ValueType::AllocatorType&)
    {
        return v.SetUint(data);
    }
};
...
template <typename ValueType> struct TypeHelper<ValueType, int64_t> {
    ...
    static ValueType& Set(
        ValueType& v, int64_t data, typename ValueType::AllocatorType&)
    {
        return v.SetInt64(data);
    }
};
...
template <typename ValueType> struct TypeHelper<ValueType, uint64_t> {
    ...
    static ValueType& Set(
        ValueType& v, uint64_t data, typename ValueType::AllocatorType&)
    {
        return v.SetUint64(data);
    }
};
...
template <typename ValueType> struct TypeHelper<ValueType, double> {
    ...
    static ValueType& Set(
        ValueType& v, double data, typename ValueType::AllocatorType&)
    {
        return v.SetDouble(data);
    }
};
...
template <typename ValueType> struct TypeHelper<ValueType, float> {
    ...
    static ValueType& Set(
        ValueType& v, float data, typename ValueType::AllocatorType&)
    {
...
struct TypeHelper<ValueType, typename ValueType::Array> {
    ...
    static ValueType& Set(
        ValueType& v, ArrayType data, typename ValueType::AllocatorType&)
    {
        return v = data;
    }
};
...
struct TypeHelper<ValueType, typename ValueType::Object> {
    ...
    static ValueType& Set(
        ValueType& v, ObjectType data, typename ValueType::AllocatorType&)
    {
        v = data;
    }
};
...
public:
    ...
    typedef Allocator
        AllocatorType; //!< Allocator type from template parameter.
...
public:
    ...
    */
    explicit GenericValue(Type type) RAPIDJSON_NOEXCEPT : data_()
    {
        static const uint16_t defaultFlags[7]
            = { kNullFlag, kFalseFlag, kTrueFlag, kObjectFlag, kArrayFlag,
                  kShortStringFlag, kNumberAnyFlag };
    ...
    */
    ...
    {
        data_.n.i64 = i;
        data_.f.flags = (i >= 0) ? (kNumberIntFlag | kUintFlag | kUint64Flag)
                                 : kNumberIntFlag;
    }
    ...
    */
    ...
    /*! \param str Constant string reference to be assigned
       ...
       primitive type assignment overload below. \see GenericStringRef,
       operator=(T)
    */
    ...
    /*! \tparam T Either \ref Type, \c int, \c unsigned, \c int64_t, \c uint64_t
        ...
        \note The source type \c T explicitly disallows all pointer types,
            ...
            \ref StringRef() (to explicitly mark the pointer as constant)
       instead. All other pointer types would implicitly convert to \c bool, use
       \ref SetBool() instead.
    */
    template <typename T>
    ...
    {
        RAPIDJSON_ASSERT(
            static_cast<void*>(this) != static_cast<void const*>(&rhs));
        this->~GenericValue();
    ...
    */
    ...
    /*!
       ...
       of all values in the subtree and total lengths of all strings).
    */
    template <typename SourceAllocator>
    ...
    /*! \pre IsObject() == true
        \tparam T Either \c Ch or \c const \c Ch (template used for
       disambiguation with \ref operator[](SizeType)) \note In version 0.1x, if
       the member is not found, this function returns a null value. This makes
       issue 7. Since 0.2, if the name is not correct, it will assert. If user
       is unsure whether a member exists, user should use HasMember() first. A
    ...
    */
    ...
    /*! \pre IsObject() == true
       ...
       does not need a StrLen(). And it can also handle strings with embedded
       null characters.
    ...
    */
    ...
    /*!
        ...
        \note It is better to use FindMember() directly if you need the obtain
       the value as well. \note Linear time complexity.
    */
    ...
    */
    ...
    /*!
       ...
       handle string with null character. \param name Member name to be
       searched. \pre IsObject() == true \return Whether a member with that name
       exists. \note It is better to use FindMember() directly if you need the
       obtain the value as well. \note Linear time complexity.
    */
    template <typename SourceAllocator>
    ...
    */
    ...
    /*!
        This version is faster because it does not need a StrLen(). It can also
       handle string with null character. \param name Member name to be
       searched. \pre IsObject() == true \return Iterator to member, if it
       exists. Otherwise returns \ref MemberEnd().
    ...
    */
    ...
    /*! \param name A string value as name of member.
        ...
        \param allocator    Allocator for reallocating memory. It must be the
       same one as used before. Commonly use GenericDocument::GetAllocator().
        \return The value itself for fluent API.
        \note The ownership of \c name and \c value will be transferred to this
       object on success. \pre  IsObject() && name.IsString() \post
        ...
        if (o.size >= o.capacity) {
            if (o.capacity == 0) {
                ...
                SetMembersPointer(reinterpret_cast<Member*>(allocator.Realloc(
                    GetMembersPointer(), oldCapacity * sizeof(Member),
    ...
    */
    GenericValue& AddMember(
        GenericValue& name, std::basic_string<Ch>& value, Allocator& allocator)
    {
    ...
    */
    ...
    /*! \param name A constant string reference as name of member.
       ...
       same one as used before. Commonly use GenericDocument::GetAllocator().
        \return The value itself for fluent API.
        \note The ownership of \c value will be transferred to this object on
       success. \pre  IsObject() \post value.IsNull() \note Amortized Constant
       time complexity.
    */
    GenericValue& AddMember(
        StringRefType name, GenericValue& value, Allocator& allocator)
    ...
    */
    ...
    /*! This function do not deallocate memory in the object, i.e. the capacity
       is unchanged. \note Linear time complexity.
    */
    ...
    */
    ...
    /*! \param pos iterator to the member to remove
        \pre IsObject() == true && \ref MemberBegin() <= \c pos < \ref
       MemberEnd() \return Iterator following the removed element. If the
       iterator \c pos refers to the last element, the \ref MemberEnd() iterator
       is returned. \note This function preserves the relative order of the
       remaining object members. If you do not need this, use the more efficient
       \ref RemoveMember(MemberIterator). \note Linear time complexity.
    */
    ...
    /*! \param first iterator to the first member to remove
       ...
       \ref MemberEnd() \return Iterator following the last removed element.
        ...
        \note Linear time complexity.
    */
    MemberIterator EraseMember(
        ConstMemberIterator first, ConstMemberIterator last)
    {
        RAPIDJSON_ASSERT(IsObject());
        RAPIDJSON_ASSERT(data_.o.size > 0);
        RAPIDJSON_ASSERT(GetMembersPointer() != 0);
    ...
    */
    ...
    /*! This function do not deallocate memory in the array, i.e. the capacity
       is unchanged. \note Linear time complexity.
    */
    void Clear()
    {
    ...
    */
    ...
    /*! \param value        Constant string reference to be appended.
        \param allocator    Allocator for reallocating memory. It must be the
       same one used previously. Commonly use GenericDocument::GetAllocator().
        \pre IsArray() == true
        \return The value itself for fluent API.
        \note If the number of elements to be appended is known, calls Reserve()
    ...
    */
    ...
    const Ch* GetString() const
    {
        ...
        return (data_.f.flags & kInlineStrFlag) ? data_.ss.str
                                                : GetStringPointer();
    }
    ...
    /*! This version has better performance with supplied length, and also
       support string containing null character. \param s source string pointer.
        \param length The length of source string, excluding the trailing null
       terminator. \return The value itself for fluent API. \post IsString() ==
       true && GetString() == s && GetStringLength() == length \see
    ...
    */
    ...
    /*! This version has better performance with supplied length, and also
       support string containing null character. \param s source string. \param
       length The length of source string, excluding the trailing null
       terminator. \param allocator Allocator for allocating copied buffer.
       Commonly use GenericDocument::GetAllocator(). \return The value itself
       for fluent API. \post IsString() == true && GetString() != s &&
       strcmp(GetString(),s) == 0 && GetStringLength() == length
    */
    ...
    /*! \param s source string.
       ...
       GenericDocument::GetAllocator(). \return The value itself for fluent API.
        \post IsString() == true && GetString() != s && strcmp(GetString(),s) ==
    ...
    */
    ...
    /*! This function adopts the GoF visitor pattern.
        Typical usage is to output this JSON value as JSON text via Writer,
       which is a Handler. It can also be used to deep clone this value via
    ...
    template <typename Handler> bool Accept(Handler& handler) const
    {
        ...
        case kObjectType:
            ...
            for (ConstMemberIterator m = MemberBegin(); m != MemberEnd(); ++m) {
                RAPIDJSON_ASSERT(
                    m->name.IsString()); // User may change the type of name by
                                         // MemberIterator.
...
private:
    ...
    // implementation detail: ShortString can represent zero-terminated strings
    // up to MaxSize chars (excluding the terminating zero) and store a value to
    // determine the length of the contained string in the last character
    // str[LenPos] by storing "MaxSize - length" there. If the string to store
    // has the maximal length of MaxSize then str[LenPos] will be 0 and
    // therefore act as the string terminator as well. For getting the string
    // length back from that value just use "MaxSize - str[LenPos]". This allows
    // to store 13-chars strings in 32-bit mode, 21-chars strings in 64-bit
    // mode, 13-chars strings for RAPIDJSON_48BITPOINTER_OPTIMIZATION=1 inline
    ...
    // By using proper binary layout, retrieval of different integer types do
    // not need conversions.
    union Number {
    ...
    void SetStringRaw(StringRefType s, Allocator& allocator)
    {
        ...
        if (ShortString::Usable(s.length)) {
            ...
            data_.s.length = s.length;
            str = static_cast<Ch*>(
                allocator.Malloc((s.length + 1) * sizeof(Ch)));
...
public:
    ...
    */
    ...
    GenericDocument& operator=(GenericDocument&& rhs) RAPIDJSON_NOEXCEPT
    {
        ...
        // Calling the destructor here would prematurely call stack_'s
        // destructor
        Destroy();
    ...
    */
    ...
    GenericDocument& Parse(
        const typename SourceEncoding::Ch* str, size_t length)
    {
        RAPIDJSON_ASSERT(!(parseFlags & kParseInsituFlag));
    ...
    {
        // c_str() is constant complexity according to standard. Should be
        // faster than Parse(const char*, size_t)
...
public:
    ...
    bool EndObject(SizeType memberCount)
    {
        ...
        stack_.template Top<ValueType>()->SetObjectRaw(
            members, memberCount, GetAllocator());
        return true;
    }
    ...
    bool EndArray(SizeType elementCount)
    {
        ...
        stack_.template Top<ValueType>()->SetArrayRaw(
            elements, elementCount, GetAllocator());
        return true;
    }
    ...
    void ClearStack()
    {
        if (Allocator::kNeedFree)
            while (stack_.GetSize()
                > 0) // Here assumes all elements in stack array are
                     // GenericValue (Member is actually 2 GenericValue objects)
...
public:
    ...
    MemberIterator EraseMember(
        ConstMemberIterator first, ConstMemberIterator last) const
    {
