...
template <typename CharType = wchar_t> struct UTF16 {
    ...
    static void EncodeUnsafe(OutputStream& os, unsigned codepoint)
    {
        ...
        if (codepoint <= 0xFFFF) {
            ...
            unsigned v = codepoint - 0x10000;
            PutUnsafe(
                os, static_cast<typename OutputStream::Ch>((v >> 10) | 0xD800));
...
#define RAPIDJSON_ENCODINGS_FUNC(x)                                            \
    ...
    template <typename OutputStream>
    RAPIDJSON_FORCEINLINE static void Encode(
        OutputStream& os, unsigned codepoint)
    {
    ...
    template <typename OutputStream>
    RAPIDJSON_FORCEINLINE static void EncodeUnsafe(
        OutputStream& os, unsigned codepoint)
    {
        typedef void (*EncodeFunc)(OutputStream&, unsigned);
    ...
    template <typename InputStream>
    RAPIDJSON_FORCEINLINE static bool Decode(
        InputStream& is, unsigned* codepoint)
    {
    ...
    template <typename InputStream, typename OutputStream>
    RAPIDJSON_FORCEINLINE static bool Validate(
...
template <typename SourceEncoding, typename TargetEncoding> struct Transcoder {
    ...
    template <typename InputStream, typename OutputStream>
    RAPIDJSON_FORCEINLINE static bool Transcode(
        InputStream& is, OutputStream& os)
    {
        unsigned codepoint;
    ...
    template <typename InputStream, typename OutputStream>
    RAPIDJSON_FORCEINLINE static bool TranscodeUnsafe(
        InputStream& is, OutputStream& os)
    {
        unsigned codepoint;
        if (!SourceEncoding::Decode(is, &codepoint))
    ...
    {
        return Transcode(is,
            os); // Since source/target encoding is different, must transcode.
    }
};
...
template <typename Encoding> struct Transcoder<Encoding, Encoding> {
        ...
        InputStream& is, OutputStream& os)
    {
        return Encoding::Validate(
            is, os); // source/target encoding are the same
    }
};
