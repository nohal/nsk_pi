...
public:
    ...
    */
    ...
    /*!
       ...
       character within [0, length - 1] range. \param length Length of the json.
...
#undef Z16
    ...
    bool ScanWriteUnescapedString(
        GenericStringStream<SourceEncoding>& is, size_t length)
    {
    ...
    void Prefix(Type type)
    {
        ...
        if (RAPIDJSON_LIKELY(
            ...
            if (!level->inArray && level->valueCount % 2 == 0)
                RAPIDJSON_ASSERT(
                    type == kStringType); // if it's in object, then even number
                                          // should be a name
            level->valueCount++;
        } else {
            RAPIDJSON_ASSERT(
...
template <>
inline bool Writer<StringBuffer>::ScanWriteUnescapedString(
    StringStream& is, size_t length)
{
    if (length < 16)
        return RAPIDJSON_LIKELY(is.Tell() < length);
